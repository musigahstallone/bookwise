...
function SpecificOrderPageContent() {
  ...
  const fetchOrderDetails = useCallback(async (showLoadingSpinner = true) => {
    if (!currentUser || !orderId) {
      ...
      if(showLoadingSpinner) setIsLoadingPage(false);
      setIsRefreshingStatus(false);
      return;
    }
    if(showLoadingSpinner) setIsLoadingPage(true);
        ...
        <CardContent className="p-4 sm:p-6">
          {order.status === 'pending' && (
            <div className="mb-6 p-4 bg-yellow-100/70 border-l-4 border-yellow-400 rounded-md text-yellow-800">
              ...
              <Button variant="outline" size="sm" onClick={() => fetchOrderDetails(false)} className="mt-3 border-yellow-500 text-yellow-700 hover:bg-yellow-200" disabled={isRefreshingStatus}>
                {isRefreshingStatus ? <Loader2 className="mr-2 h-4 w-4 animate-spin"/> : <RefreshCw className="mr-2 h-4 w-4"/>} 
                Refresh Status
              </Button>
              <Button asChild size="sm" className="mt-3 ml-2 bg-yellow-500 hover:bg-yellow-600 text-white">
